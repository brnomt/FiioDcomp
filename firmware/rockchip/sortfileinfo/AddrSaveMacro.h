...
#define  SORT_FILENUM_DEFINE        (8192)//2048      /* 限定文件最大数,文件信息保存占用4个block，每个block拥有(SecPerBlock[0] / SecPerPage[0])个page ，一个page可以保存4个文件信息 */
#define  MEDIA_ID3_SAVE_CHAR_NUM    (SYS_SUPPROT_STRING_MAX_LEN + 1)   //40 /* 保存的ID3_TITLE字符数 */
...
#define     MUSIC_SAVE_SECTOR_SIZE              (MEDIAINFO_BLOCK_SIZE * 32) /*file information occupy memory, 2048k is 2M */
...
#define     FAVORITE_MUSIC_INFO_SECTOR_START    (RECORD_TREE_INFO_SECTOR_START +  RECORD_TREE_SECTOR_SIZE)   /*start address of the keeping favorite infomation*/
...
typedef __packed struct _SORTINFO_STRUCT
{
    ...
    UINT16 ItemBaseID;      // UI获取条目显示信息时用，系统从文件列表中对应的
                            // ItemBaseID个文件中获取需要的显示信息（如//ItemBaseID这个文件对应的专辑，歌手，文件名等）
