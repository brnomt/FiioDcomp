...
{
    SDC0 = 0,                 //use only for sido, sd function,can not use the emmc function
    SDC1,                     //use for emmc ,sido, sd 
    SDC_MAX
}SDMMC_PORT_E;

#if (SDCARD_PORT == 0)
#define SDM_SD_ID   SDC0
#else //#if (SDCARD_PORT == 1)
#define SDM_SD_ID   SDC1
#endif
#define SDM_EMMC_ID SDC1     //这个宏定义值, 必须与SDMMC_PORT_E枚举定义的值相一致

typedef enum SDMMC_PORT_FUNC
{
    SD_CARD_FUNC = 0,                 //use only for sd card function
    EMMC_FUNC,                     //use for emmc function
}SDMMC_PORT_FUNCTION;
