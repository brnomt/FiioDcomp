...
void Grf_I2S0_Sel(eI2S_SELECT sel)
{
    ...
    else if( I2S_SEL_ACODEC == sel)
        Grf->GRF_IOMUX_CON = I2S0_SEL_MASK << 16 | I2S0_SEL_ACODEC;
}

void Grf_I2S1_Sel(eI2S_SELECT sel)
{
    if( I2S_SEL_APAD == sel)
        Grf->GRF_IOMUX_CON = I2S1_SEL_MASK << 16 | I2S1_SEL_APAD;
    else if(I2S_SEL_BPAD == sel)
        Grf->GRF_IOMUX_CON = I2S1_SEL_MASK << 16 | I2S1_SEL_BPAD;
}
...
void Grf_otgphy_suspend(BOOL Enable)
{
    if (Enable)
    {
        Grf->GRF_UOC_CON1 = (USB_CTRL_SEL_MASK << 16) | USB_CTRL_SEL_NON_PHY;   //USB control enable
        Grf->GRF_UOC_CON1 = ((UTMI_SUSPEND_N_MASK | UTMI_OPMODE_MASK) << 16)
                            | UTMI_SUSPEND_N_SUSPEND | UTMI_OPMODE_2; //0x000f0005
    ...
    else
    {
        ...
        Grf->GRF_UOC_CON1 = (USB_CTRL_SEL_MASK << 16) | USB_CTRL_SEL_PHY;   //USB control enable
    }
...
void Grf_Otgphy_Driver_Confige(void)
{
    ...
    Grf->GRF_USBPHY_CON1 = (((HS_EYE_HEIGHT_HI5BIT_MASK  << 16) | 0x01) |  //bit [0] [15:13]
                            ((HS_EYE_HEIGHT_LOW3BIT_MASK << 16) | 0x03));  //      1   100
    Grf->GRF_USBPHY_CON3 = (BIAS_CURRENT_REF_MASK << 16) | BIAS_CURRENT_REF_425MV;
