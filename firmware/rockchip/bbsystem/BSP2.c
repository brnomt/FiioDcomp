...
extern uint32  BBSystemHoldTimeout;
...
__irq void BBSystemBIsr(void)
{
    ...
    switch(Cmd)
    {
        case MSGBOX_CMD_BB_HOLD:
            ...
            MailBoxWriteB2ACmd(MSGBOX_CMD_BB_HOLD_ACK,  MAILBOX_ID_0, MAILBOX_CHANNEL_0);
            ...
            while(1)
            {
                DelayUs2(5);
                Cmd = MailBoxReadA2BCmd(MAILBOX_ID_0, MAILBOX_CHANNEL_0);
                if (Cmd == MSGBOX_CMD_BB_HOLD_EXIT)
                {
                    MailBoxClearA2BInt(MAILBOX_ID_0,  MAILBOX_INT_0);
                    break;
                }
                if (--counter == 0)
                {
                    BBSystemHoldTimeout = 1;
                    break;
                }
            }
            break;

        case MSGBOX_CMD_BB_HOLD_EXIT:
            break;
    }
}

__irq void BBSystemDebugBIsr(void)
{
    uint32 Cmd;
    uint32 Data;

    MailBoxClearA2BInt(MAILBOX_ID_0,  MAILBOX_INT_3);
...
void BSP_Init2(void)
{
    ...
    MailBoxEnableA2BInt(MAILBOX_ID_0, MAILBOX_INT_3);
    IntRegister2(INT_ID_MAILBOX3, (void*)BBSystemDebugBIsr);
    IntPendingClear2(INT_ID_MAILBOX3);
    IntEnable2(INT_ID_MAILBOX3);
