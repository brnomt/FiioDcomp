        ...
        if (iFrameInfo != ID3_NONINFO)
        {
            ...
            switch (iFrameInfo)
            {
                ...
                case ID3_GENRE:
                    ...
                    {
                        Ascii2Unicode((UINT16*)ID3_GenreTable[GenreID], (UINT16*)(pstId3v2x->id3_genre),23, ID3_ENCODE_UTF8);  //最长23个字节
                    }
                    break;
...
static int APEV2_Parse(HANDLE fHandle, ID3V2X_INFO * pstId3v2x, unsigned char *tempbuf)
{
    ...
    if(FileRead((uint8*)&size, 4, fHandle) < 4) return -1;      //所有标签帧和标签尾的总长度, 不包含APE Tags Header
    if(FileRead((uint8*)&fields, 4, fHandle) < 4) return -1;    //标签帧个数
    if(FileRead((uint8*)&flag, 4, fHandle) < 4) return -1;

    if(fields <= 65536 && (flag & APE_TAG_FLAG_CONTAINS_HEADER))
    {
        FileSeek(8, FSEEK_CUR, fHandle);    //skip, 8byte保留字节，全为0

        for(i = 0; i < fields; i++)
        {
            if(FileRead((uint8*)&taginfolen, 4, fHandle) < 4) return -1; //帧内容长度
            FileSeek(4, FSEEK_CUR, fHandle);    //skip，4byte填充标记
            ...
            if(tagid != TAGID_NULL)
            {
                ...
                if(FileRead(tempbuf, taginfolen, fHandle) < taginfolen) return -1;  //读取帧内容
                readsize += taginfolen;
                if(readsize > size) break;
...
int APEInfo_Parse(HANDLE fHandle, ID3V2X_INFO * pstId3V2x)
{
    ...
    if(FileRead((uint8*)&version, 4, fHandle) < 4) return -1;
    if(FileRead((uint8*)&size, 4, fHandle) < 4) return -1;
    if(FileRead((uint8*)&fields, 4, fHandle) < 4) return -1;
    if(FileRead((uint8*)&flag, 4, fHandle) < 4) return -1;
...
static int vorbis_comment_parse(HANDLE fHandle, ID3V2X_INFO * pstId3v2x, unsigned char *tempbuf, unsigned int packet_size)
{
    ...
    while(1)
    {
        ...
        {
            if(taginfolen > MAX_ID3_FIND_SIZE) return -1;   //tag content is too long
            if(FileRead(tempbuf, taginfolen, fHandle) < taginfolen) return -1;  //读取标签内容
            readsize += taginfolen;
            if(readsize > commentslen) break;
...
int OGGInfo_Parse(HANDLE fHandle, ID3V2X_INFO * pstId3V2x)
{
    ...
    while(1)
    {
        ...
        WatchDogReload();
#endif
        if(FileRead(buf, 4, fHandle) < 4) return -1; //读取页标识
