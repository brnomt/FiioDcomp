...
int m4afileformflag;
int dec_err_count;
extern int wrongframesize;
uint32  videosamplesum;
//#define TEST
        ...
        case SUBFN_CODEC_DECODE:
            {
                ...
                else
                {
                    ...
                    {
                        {
                            left = INREAD - wrongframesize;
                            //aac_MemSet(AACoutBuf, 0, 2 * SRC_Num_Forehead * sizeof(short));
                            memset(&AACoutBuf[select][SRC_Num_Forehead], 0, 1024 * sizeof(long));
                            memmove(rdBuf, pb + wrongframesize, left);
                            size = sizeof(rdBuf)-left;
                            p = rdBuf + left;
                        }

                        dec_err_count++;    //错误帧连续错误

                        if (dec_err_count > 200)    //当错误帧数达到200帧时，判错退出
                        {
                            aac_printf("错误帧数超200\n");
        ...
        case SUBFN_CODEC_OPEN_DEC:
            {
                ...
                if (m4a==0)
                {
                    if (InitTimeForADTS( rawfile, RKFIO_FLength(pRawFileCache)- fl,fl) == -3)//为什么是-3?无此返回值
                    {
                        return ERROR;
                    }
                    InitAACDataTable();
