...
extern int asf_read(uint32_t *ctx, uint32_t **out, uint32_t lo, uint32_t hi); /* FUN_03083d48 */
extern int guid_eq(const void *a, const void *b, int n);                      /* FUN_0308283c */
...
int wma_audio_parse(uint32_t *param_1, int param_2)
{
    ...
    n = asf_read(param_1, &blob, param_1[0], param_1[1]);
    if (n != 0x36)
        return 3;
    ...
    if (guid_eq(g_guid_header_obj, /* reconstructed GUID0 */ blob - 0x36 / 4, 0x10) == 0) {
    ...
    if (guid_eq(*g_guid_stream_obj, /* GUID0 */ blob - 0x36 / 4, 0x10) != 0) {
    ...
    if (fmt_size != 0) {
        int ext_len;
        if (guid_eq(*g_guid_ext_a, /* GUID1 */ NULL, 0x10) == 0)
            ext_len = 9;
        else if (guid_eq(*g_guid_ext_b, NULL, 0x10) == 0)
            ext_len = 8;
        else
            return 1;
