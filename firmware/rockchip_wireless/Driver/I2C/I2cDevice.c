...
BOOL I2C_Init(eI2C_DEV i2cDev,uint16 speed);
...
//    gpstI2cDevHandler[i2cDev] = pstI2CDev;
...
/*
    ...
    {
        I2CSetSpeed(i2cDev,Rx_arg->speed);
        pstI2CDev->speed = Rx_arg->speed;
    }
*/
    rkos_exit_critical();

    I2CDisableIRQ(i2cDev); //clear all irq
    ...
    I2CEnableInt(i2cDev, I2C_MBRFIEN | I2C_NAKRCVIEN);
    ...
    if ( TimeOut )
    {
        ...
        if ( gI2C_intType & I2C_NAKRCVIPD )
        {
            ...
            I2C_Deinit(i2cDev);
            I2CDisableInt(i2cDev, I2C_MBRFIEN | I2C_NAKRCVIEN);
...
COMMON FUN void I2CDevIntIsr(uint32 i2cDevID)
{
    ...
    if ( intpending & I2C_MBTFIPD ) //??????
    {
        if ( DEVICE_STATUS_SYNC_PIO_WRITE == pstI2CDev->dwTxStatus)
        {
            ...
            {
                //printf("2222 osI2CWriteSem give\n");
                rkos_semaphore_give_fromisr(pstI2CDev->osI2CWriteSem);
                pstI2CDev->dwTxStatus = DEVICE_STATUS_IDLE;
                I2CStop(i2cDevID);
                I2C_Deinit(i2cDevID);
...
COMMON API rk_size_t I2cDev_SendData(HDC dev,uint8* DataBuf,uint32 size,I2C_CMD_ARG *Tx_arg)
{
    ...
    i2cDev = pstI2CDev->stI2CDevice.DevID;
    //rk_printf("osI2CWriteReqSem take\n");
...
/*
    ...
    {
        I2CSetSpeed(i2cDev,Tx_arg->speed);
...
*/
    ...
    I2CEnableInt(i2cDev, I2C_MBTFIEN | I2C_NAKRCVIEN);
    //rk_printf("I2CEnableInt\n");
    ...
    if (TimeOut)
    {
        ...
        if ( gI2C_intType & I2C_NAKRCVIPD ) //ACK
        {
            rk_printf("\n------I2C_NAKRCVIPD------\n");
            I2CStop(i2cDev );
            while(!gI2C_flag)
            ...
            I2C_Deinit(i2cDev);
            I2CDisableInt(i2cDev, I2C_MBTFIEN | I2C_NAKRCVIEN);
            return ERROR;   //need retry to send again.
        }
    }
    else
...
INIT API rk_err_t I2CDev_Delete(uint32 DevID, void * arg)
{
    ...
    rkos_semaphore_delete( gpstI2cDevHandler[DevID]->osI2CReadReqSem);
    rkos_semaphore_delete( gpstI2cDevHandler[DevID]->osI2CWriteReqSem);
    rkos_semaphore_delete( gpstI2cDevHandler[DevID]->osI2CReadSem);
    rkos_semaphore_delete( gpstI2cDevHandler[DevID]->osI2CWriteSem);
...
INIT FUN rk_err_t I2CDevInit(HDC dev)
{
    ...
    //I2C_Init(pstI2CDevHandler->stI2CDevice.DevID, pstI2CDevHandler->speed);
    rk_printf("I2CDevInit FUN DevID=%d speed=%d\n",pstI2CDevHandler->stI2CDevice.DevID, pstI2CDevHandler->speed );
...
SHELL FUN rk_err_t I2CDevShellPcb(HDC dev, uint8 * pstr)
{
 uint32 DevID;
    ...
    if(gpstI2cDevHandler[DevID] != NULL)
    {
        ...
        rk_printf_no_time("    .osI2CReadReqSem = %08x",pstI2cDev->osI2CReadReqSem);
        rk_printf_no_time("    .osI2CWriteReqSem = %08x",pstI2cDev->osI2CWriteReqSem);
        rk_printf_no_time("    .osI2CReadSem = %08x",pstI2cDev->osI2CReadSem);
        rk_printf_no_time("    .osI2CWriteSem = %08x",pstI2cDev->osI2CWriteSem);
...
SHELL FUN rk_err_t I2CDevShellRead(HDC dev, uint8 * pstr)
{
    ...
    I2C_Init(I2C_DEV0,RTC_HYM8563_IICSpeed);
