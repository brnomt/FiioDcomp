...
{
    int totalSize, reqSize, offset= 0;
    ...
    pstBuf= (uint8 *)rkos_memory_malloc(totalSize);
    while( totalSize )
    {
        reqSize= (totalSize > (4088*4))? (4088*4) : totalSize;
        if(FW_GetPicResource(offsetAddr, &pstBuf[offset], reqSize)!= RK_SUCCESS)
        {
    ...
    while( totalSize > 0 )
    {
        ...
        totalSize -=  psPictureInfo->xSize* ICON_PIXEL_WIDE;
        py++;
    }
