    ...
    else if(gpstFmUiData->FMPlayerMod == MOD_REC)
    {
        switch(KeyVal)
        {
            case KEY_VAL_PLAY_SHORT_UP:
                ...
                printf("FMPlay_FMModeDisplay= %d Recordstate=%d\n",gpstFmUiData->PlayerState ,gpstFmUiData->Recordstate);
                #ifdef _USE_GUI_
            ...
            case KEY_VAL_ESC_SHORT_UP:
                    ...
                    gpstFmUiData->PlayerState = FM_PLAYER_STATE_PLAY;
                    FMPlay_StateDisplay();
                    printf("ESC rec mod..0x%x\n",gpstFmUiData->FMPlayerMod);
                #endif
