...
COMMON FUN int MusicPlayMenuTask_KeyEvent(uint32 KeyVal)
{
    ...
    switch (KeyVal)
    {
        ...
        case KEY_VAL_MENU_SHORT_UP: //Start and Stop
            ...
            else if ((gpstPlayMenuData->Arg.ucSelPlayType == SOURCE_FROM_DLNA)
    ...
    if(SourecePlayer == SOURCE_FROM_FILE_BROWSER) //Local
    {
        ...
        if(SourecePlayer == SOURCE_FROM_DLNA) //DLNA
        {
            if(wifi_join_flag() == WICED_TRUE)
            {
                ...
                rk_dlna_start();
                gSysConfig.PlayerType = SOURCE_FROM_DLNA;
                gpstPlayMenuData->Arg.ucSelPlayType = SOURCE_FROM_DLNA;
                MainTask_SetStatus(MAINTASK_APP_DLNA_PLAYER, 1);
                MainTask_SetStatus(MAINTASK_APP_DLNA_PLAYER_START, 0);
                #endif
            }
    ...
    switch (cmd)
    {
        case 0:
            ...
            while(1)
            {
                if(wifi_join_flag() == WICED_TRUE)
                {
                    rk_printf("1.wifi(smart) config ok-------------------------------\n");
                    MusicPlay_StartPlayer(gSysConfig.PlayerType);
                    break;
                }
                else if (wifi_join_flag() == WICED_ERR)
                {
                    rk_printf("1.wifi(smart) config error-------------------------------\n");
                    break;
                }
                else if(wifi_join_flag() == WICED_WAIT)
                {
                    rk_printf("1.wifi(smart) config wait-------------------------------\n");
                    rkos_sleep(2000);
                }
                else
                {
                    rk_printf("1.wifi(smart) config unknown-------------------------------\n");
                    break;
                }
            }
            //MusicPlay_StartPlayer(gSysConfig.PlayerType);
...
COMMON API void MusicPlayMenuTask_Enter(void)
{
    ...
    #ifdef _WIFI_
    {
        ...
        while(1)
        {
            if(wifi_join_flag() == WICED_TRUE)
            {
                rk_printf("wifi(smart) config ok-------------------------------\n");
                if (gpstPlayMenuData->Arg.ucSelPlayType != SOURCE_FROM_FILE_BROWSER)
                    MusicPlay_StartPlayer(gpstPlayMenuData->Arg.ucSelPlayType);
            ...
            {
                rk_printf("wifi(smart) config error-------------------------------\n");
                break;
            }
            else if(wifi_join_flag() == WICED_WAIT)
            {
                rk_printf("wifi(smart) config wait-------------------------------\n");
                rkos_sleep(2000);
            }
            else
            {
                rk_printf("wifi(smart) config unknown-------------------------------\n");
                break;
            }
        }
    }
