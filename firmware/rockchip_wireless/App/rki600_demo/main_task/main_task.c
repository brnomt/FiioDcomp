...
int32 MainTask_Animation (int params);
...
COMMON API void MainTask_AudioCallBack(uint32 audio_state)
{
    ...
    if (audio_state == AUDIO_STATE_ERROR)
    {
        ...
        MainAskQueue.event = MAINTASK_AUDIO_ERROR;

        ret = rkos_queue_send(gpstMainData->MainAskQueue, &MainAskQueue, MAX_DELAY);
        if (ret == RK_ERROR)
        {
            MAINTASK_DEBUG ("---------lose AUDIO_STATE_ERROR\n");
        }
    }
    return ;
}
...
*******************************************************************************/
_APP_MAIN_TASK_MAIN_TASK_COMMON_
COMMON int32 MainTask_NoScreenUSBKey(uint32 keyvalue)
{
    printf ("--------app is: USB\n");
    switch (keyvalue)
    {
...
int32 MainTask_HandleKey(uint32 event)
{
    ...
    switch (event)
    {
        ...
        case KEY_VAL_FFW_SHORT_UP:
            gpstMainData->MenuId++;
            #if 1 //wrm-add linshi
            if ((gpstMainData->MenuId>6)&&(gpstMainData->MenuId<MenuIdMax))
            {
...
int32 MainTask_HandleSysEvent(uint32 event)
{
    ...
    switch (event)
    {
        ...
        case MAINTASK_AUDIO_ERROR:
            MAINTASK_DEBUG ("Song ---------ERROR(MainTask)\n");
            MainTask_SetTopPrompt(0);
...
*******************************************************************************/
_APP_MAIN_TASK_MAIN_TASK_COMMON_
COMMON FUN int32 MainTask_SetTopPrompt (int params)
{
    if (params == 0) //song error
    {
...
*******************************************************************************/
_APP_MAIN_TASK_MAIN_TASK_MENU_
COMMON FUN int32 MainTask_Animation (int params)
{
#ifdef _USE_GUI_
    int i;
    HGC hGc;
    ...
    for (i=0; i<23; i++)
    {
        pGcArg.resource= i + IMG_ID_POWERON0;
        hGc= GUITask_CreateWidget(GUI_CLASS_ICON, &pGcArg);
        if (hGc== NULL)
        {
            MAINTASK_DEBUG("Create Icon Class fault\n");
            return RK_ERROR;
        }
...
COMMON FUN int32 MainTask_ShutDown (int flag)
{
    ...
    for (i=0; i<11; i++)
    {
        pGcArg.resource= i + IMG_ID_POWEROFF0;
        hGc= GUITask_CreateWidget(GUI_CLASS_ICON, &pGcArg);
        if (hGc== NULL)
        {
            MAINTASK_DEBUG("Create Icon Class fault\n");
...
COMMON FUN int32 MainTask_DeleteAllApp (void)
{
    ...
    if (MsgDev_GetMsg(gpstMainData->hMsg, MAINTASK_WIFI_OK) == RK_SUCCESS)
    {
        #ifdef _DRIVER_WIFI__
...
COMMON FUN int32 MainTask_SetName (int32 MenuId)
{
    ...
    switch(MenuId)
    {
        ...
        case 3:
            ...
            break;
        case 4:
            pstTextArg.resource = SID_5_1_CHANNEL;
            pHgc = GUITask_CreateWidget(GUI_CLASS_TEXT, &pstTextArg);
            break;
        case 5:
...
COMMON FUN int32 MainTask_StartSubThread (uint32 MenuId)
{
    ...
    switch (MenuId)
    {
        case MUSIC_SUB_THREAD: //本地播放器
            ...
            MainTask_TaskSwtich(TASK_ID_MAIN, 0, TASK_ID_MEDIA_LIBRARY,0, NULL);
#else
