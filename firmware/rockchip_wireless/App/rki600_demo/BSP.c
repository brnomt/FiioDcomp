...
#define HEAP_HEAD_MARK_FLAG (0xAA55AA55)
/*
*---------------------------------------------------------------------------------------------------------------------
*
...
__attribute__((section("bsp_ApMainHeadflag"))) uint32 APMainStackHeadFlag=0;
__attribute__((section("bsp_HeapStackHeadflag"))) uint32 HeapStackHeadFlag=0;
__attribute__((section("bsp_AppHeapHeadflag"))) uint32 AppHeapHeadFlag=0;
__attribute__((section("bsp_AppHeapTailflag"))) uint32 AppHeapTailFlag=0;
#ifdef _LOG_DEBUG_
...
*/
/*******************************************************************************
** Name: sys_check_heap_block_overflow
** Input:void
** Return: rk_err_t
** Owner:wangping
** Date: 2016.4.28
...
COMMON API rk_err_t sys_check_heap_block_overflow(void)
{
    if(APMainStackHeadFlag != HEAP_HEAD_MARK_FLAG)
    {
        printf("APMainStackHeadFlag be rewrite\n");
    }

    if(HeapStackHeadFlag != HEAP_HEAD_MARK_FLAG)
    {
        printf("HeapStackHeadFlag be rewrite\n");
    }

    if(AppHeapHeadFlag != HEAP_HEAD_MARK_FLAG)
    {
        printf("AppHeapHeadFlag be rewrite\n");
    }

    if(AppHeapTailFlag != HEAP_HEAD_MARK_FLAG)
    {
        printf("AppHeapTailFlag be rewrite\n");
    }
}
/*******************************************************************************
** Name: PnPSever
...
COMMON API void PnPSever(void)
{
    ...
    if(VbusStatus == 0)
    {
        ...
        if(Grf_CheckVbus()&&(VbusTimeCount>=5))
        {
            printf ("\n-----PnPSever connect -------\n");
    ...
    else if(VbusStatus == 1)
    {
        ...
        if((Grf_CheckVbus() == 0)&&(VbusTimeCount>=10))
        {
...
extern volatile portTickType xLastTaskSwtichTime;
_BSP_EVK_V20_BSP_COMMON_
...
//    Grf_GPIO_SetPinPull(GPIO_CH2, GPIOPortA_Pin4, DISABLE);
    ...
    Gpio_SetPinDirection(WIFI_SPI_INT_PORT, WIFI_SPI_INT, GPIO_OUT);
    Grf_GPIO_SetPinPull(WIFI_SPI_INT_PORT, WIFI_SPI_INT, DISABLE);
    Gpio_SetPinLevel(WIFI_SPI_INT_PORT, WIFI_SPI_INT, GPIO_LOW);

    Grf_GpioMuxSet(GPIO_CH2, GPIOPortC_Pin1, 0);
    Grf_GpioMuxSet(GPIO_CH2, GPIOPortC_Pin0, 0);
    Grf_GpioMuxSet(GPIO_CH2, GPIOPortB_Pin6, 0);
    Grf_GpioMuxSet(GPIO_CH2, GPIOPortB_Pin7, 0);

    Gpio_SetPinDirection(GPIO_CH2, GPIOPortC_Pin1, GPIO_OUT);
    Gpio_SetPinDirection(GPIO_CH2, GPIOPortC_Pin0, GPIO_OUT);
    Gpio_SetPinDirection(GPIO_CH2, GPIOPortB_Pin6, GPIO_OUT);
    Gpio_SetPinDirection(GPIO_CH2, GPIOPortB_Pin7, GPIO_OUT);
    ...
    Gpio_SetPinLevel(GPIO_CH2, GPIOPortC_Pin1, GPIO_LOW);
    Gpio_SetPinLevel(GPIO_CH2, GPIOPortC_Pin0, GPIO_LOW);
    Gpio_SetPinLevel(GPIO_CH2, GPIOPortB_Pin6, GPIO_LOW);
    Gpio_SetPinLevel(GPIO_CH2, GPIOPortB_Pin7, GPIO_LOW);
#endif
}
/*******************************************************************************
** Name: CPUInit
...
INIT FUN void SysVariableInit(void)
{
    ...
    APMainStackHeadFlag=HEAP_HEAD_MARK_FLAG;
    HeapStackHeadFlag=HEAP_HEAD_MARK_FLAG;
    AppHeapHeadFlag=HEAP_HEAD_MARK_FLAG;
    AppHeapTailFlag = HEAP_HEAD_MARK_FLAG;
}
/*******************************************************************************
