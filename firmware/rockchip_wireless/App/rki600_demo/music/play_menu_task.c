...
#define MaximumProgressBar              10000
...
int      MusicPlay_PlayerOpen(int type);
...
COMMON FUN int MusicPlayMenuTask_MsgBoxEvent(uint32 cmd, int type)
{
    ...
    switch (cmd)
    {
        ...
        case KEY_VAL_FFD_LONG_UP:
            ...
            if ((gpstPlayMenuData->playerType == SOURCE_FROM_CUR_AUDIO)
               ||(gpstPlayMenuData->playerType == SOURCE_FROM_FILE_BROWSER))
            {
    ...
    switch (KeyVal)
    {
            ...
            #ifdef _BLUETOOTH_
            {
                ...
                switch (gSysConfig.PlayerType)
                {
                    case SOURCE_FROM_BT:
                        ...
                        MusicPlay_StartPlayer(SOURCE_FROM_CUR_AUDIO);
            ...
            #ifdef _WIFI_
            {
                ...
                switch (gSysConfig.PlayerType)
                {
                    case SOURCE_FROM_DLNA:
                        ...
                        if (MainTask_GetStatus(MAINTASK_APP_DLNA_PLAYER)==1)
                        {
                            if(wifi_connect_flag() == WICED_TRUE)
                            {
                    ...
                    case SOURCE_FROM_XXX:
                        ...
                        #endif
                        if (gpstPlayMenuData->AIRControl==1)
                        {
                            if(wifi_connect_flag() == WICED_TRUE)
                            {
                            ...
                            gpstPlayMenuData->AIRControl=0;
                            MainTask_SetStatus(MAINTASK_APP_XXX_PLAYER, 0);
                            gSysConfig.PlayerType = SOURCE_FROM_CUR_AUDIO;
                        }
        ...
        case KEY_VAL_ESC_SHORT_UP:  //退出播放器界面，但保留播放器
            ...
            if ((gpstPlayMenuData->playerType == SOURCE_FROM_CUR_AUDIO)
               ||(gpstPlayMenuData->playerType == SOURCE_FROM_FILE_BROWSER))
            {
                //AudioControlTask_SetStateChangeFunc(NULL);
                AudioControlTask_SetStateChangeFunc(MusicPlay_AudioCallBack, MainTask_AudioCallBack);
            }
            else
            {
                AudioControlTask_SetStateChangeFunc(MusicPlay_AudioCallBack, 0);
            }

            MusicPlay_DeleteGuiHandle();

            if (gpstPlayMenuData->playerType == SOURCE_FROM_FILE_BROWSER)
            {
                #ifdef __APP_BROWSER_BROWSERUITASK_C__
                RK_TASK_Browser_ARG Arg;
                memcpy(Arg.filepath, gpstPlayMenuData->Arg.filepath, StrLenW(gpstPlayMenuData->Arg.filepath)*2);
                Arg.filepath[StrLenW(gpstPlayMenuData->Arg.filepath)] = 0x0000;
        ...
        case KEY_VAL_ESC_PRESS: //退出播放器界面，不保留播放器
            ...
            if (((gpstPlayMenuData->playerType == SOURCE_FROM_CUR_AUDIO)
                ||(gpstPlayMenuData->playerType == SOURCE_FROM_FILE_BROWSER)))
            {
                PLAYMENU_DEBUG ("Delete player not store\n");
                MusicPlayMenuTask_DeleteSelf(1);
                while(1)
                {
                    rkos_sleep(2000);
        ...
        case KEY_VAL_FFW_SHORT_UP: //Next Song
            ...
            else if (gpstPlayMenuData->playerType == SOURCE_FROM_BT)
            {
        ...
        case KEY_VAL_FFD_SHORT_UP: // Last Song
            ...
            if(gpstPlayMenuData->playerType == SOURCE_FROM_BT)
            {
        ...
        case KEY_VAL_UP_SHORT_UP: //Volume +
            ...
            if (((gpstPlayMenuData->playerType == SOURCE_FROM_CUR_AUDIO)
                ||(gpstPlayMenuData->playerType == SOURCE_FROM_FILE_BROWSER)
                ||(gpstPlayMenuData->playerType == SOURCE_FROM_BT)))
            {
        ...
        case KEY_VAL_MENU_SHORT_UP: //Start and Stop
            ...
            else if ((gpstPlayMenuData->playerType == SOURCE_FROM_DLNA)
                    ||(gpstPlayMenuData->playerType == SOURCE_FROM_XXX))
            {
...
COMMON FUN void MusicPlay_VolumeDisplay(void)
{
    ...
    if(gpstPlayMenuData->hVolumeBar == NULL)
    {
        ...
        pstProgressArg.percentage= gpstPlayMenuData->Volume* 10000/ MaximumVolume;
    ...
    else
    {
        ...
        pstProgressBarContent.percentage = gpstPlayMenuData->Volume* 10000/ MaximumVolume;
...
COMMON FUN void MusicPlay_PlayProcessDisplay(void)
{
    ...
    pstProgressBarContent.cmd = PROGRESSBAR_SET_CONTENT;
    pstProgressBarContent.percentage = gpstPlayMenuData->MusicProgressTimer* MaximumProgressBar / gpstPlayMenuData->MusicTotalTimer;
...
*******************************************************************************/
_APP_MUSIC_PLAY_MENU_TASK_COMMON_
COMMON FUN int MusicPlay_PlayerOpen(int type)
{
    int ret = 0;
    MUSIC_PLAY_MENU_ASK_QUEUE PlayMenuAskQueue;
    ret = MusicPlay_SelectExePlayer(gpstPlayMenuData->playerType);
    if (ret == -1)  //播放器打开失败
    {
        ...
        while (1)
        {
            ...
            switch (PlayMenuAskQueue.cmd)
            {
                case KEY_VAL_ESC_SHORT_UP:
                    ...
                    else if(gpstPlayMenuData->playerType == SOURCE_FROM_CUR_AUDIO)
                    {
                        MainTask_TaskSwtich(TASK_ID_MUSIC_PLAY_MENU, 0, TASK_ID_MAIN, 0, NULL);
                    }
                    else
...
COMMON FUN rk_err_t MusicPlay_DeletePlayer(uint32 SourecePlayer)
{
    ...
    if ((SourecePlayer == SOURCE_FROM_FILE_BROWSER)
         ||(SourecePlayer == SOURCE_FROM_CUR_AUDIO))
    {
        RKTaskDelete(TASK_ID_AUDIOCONTROL, 0, SYNC_MODE);
        return RK_SUCCESS;
    }
    ...
    //Delete dlnaplayer
    if (SourecePlayer == SOURCE_FROM_DLNA)
    {
        #ifdef __WIFI_DLNA_C__
        rk_dlna_end();
        RKTaskDelete(TASK_ID_DLNA, 0, SYNC_MODE);
...
COMMON FUN rk_err_t MusicPlay_StartPlayer(uint32 SourecePlayer)
{
    ...
    else if (SourecePlayer == SOURCE_FROM_FILE_BROWSER)
    {
    ...
    else if (SourecePlayer == SOURCE_FROM_DLNA)
    {
...
#else //无屏幕操作打开播放器
    RK_TASK_AUDIOCONTROL_ARG Arg;
    if(SourecePlayer == SOURCE_FROM_CUR_AUDIO) //Local
    {
        Arg.ucSelPlayType = SOURCE_FROM_FILE_BROWSER;
        Arg.FileNum = 0;
        Arg.pfAudioState = MusicPlay_AudioCallBack;
        ...
        gSysConfig.PlayerType = SOURCE_FROM_CUR_AUDIO;
        gpstPlayMenuData->playerType = SOURCE_FROM_CUR_AUDIO;
    ...
    #endif
    #ifdef _WIFI_
        if(SourecePlayer == SOURCE_FROM_DLNA) //DLNA
        {
            if(wifi_connect_flag() == WICED_TRUE)
            {
                #ifdef __WIFI_DLNA_C__
                ...
                rk_dlna_start();
                gSysConfig.PlayerType = SOURCE_FROM_DLNA;
                gpstPlayMenuData->playerType = SOURCE_FROM_DLNA;
                MainTask_SetStatus(MAINTASK_APP_DLNA_PLAYER, 1);
                MainTask_SetStatus(MAINTASK_APP_DLNA_PLAYER_START, 0);
                #endif
            }
...
COMMON FUN rk_err_t MusicPlayMenuTask_GuiCallBack(APP_RECIVE_MSG_EVENT evnet_type, uint32 event, void * arg, HGC pGc)
{
    ...
    if ((gpstPlayMenuData->QueueFull == 1) && ((event & KEY_STATUS_LONG_UP) != KEY_STATUS_LONG_UP))
    {
        PLAYMENU_DEBUG ("lose key QueueFull=%d\n",gpstPlayMenuData->QueueFull);
        return RK_SUCCESS;
    }

    if(evnet_type==APP_RECIVE_MSG_EVENT_KEY)
    {
        gpstPlayMenuData->QueueFull = 1;
        ...
        if (ret == RK_ERROR)
        {
            if ((event==KEY_VAL_FFW_PRESS)||(event==KEY_VAL_FFD_PRESS))
            {
                gpstPlayMenuData->QueueFull = 0;
                return RK_SUCCESS;
            }
            rkos_queue_receive(gpstPlayMenuData->PlayMenuAskQueue, &PlayMenuAskQueue_tmp, 0);
            if (PlayMenuAskQueue_tmp.type != PLAYMENU_KEY_EVENT)
            {
                //PLAYMENU_DEBUG ("1:type=%d, cmd=%d\n",PlayMenuAskQueue_tmp.type,PlayMenuAskQueue_tmp.cmd);
                rkos_queue_send(gpstPlayMenuData->PlayMenuAskQueue, &PlayMenuAskQueue_tmp, 0);
                rkos_queue_receive(gpstPlayMenuData->PlayMenuAskQueue, &PlayMenuAskQueue_tmp, 0);
                if (PlayMenuAskQueue_tmp.type != PLAYMENU_KEY_EVENT)
                {
                    //PLAYMENU_DEBUG ("2:type=%d, cmd=%d\n",PlayMenuAskQueue_tmp.type,PlayMenuAskQueue_tmp.cmd);
                    rkos_queue_send(gpstPlayMenuData->PlayMenuAskQueue, &PlayMenuAskQueue_tmp, 0);
                    gpstPlayMenuData->QueueFull = 0;
                }
                else
                {
                    ...
                    if (ret == RK_ERROR)
                    {
                        PLAYMENU_DEBUG ("Send Key Failure 0\n");
                        gpstPlayMenuData->QueueFull = 0;
                    }
                }
            }
            else
            {
                ...
                if (ret == RK_ERROR)
                {
                    PLAYMENU_DEBUG ("Send Key Failure 1\n");
                    gpstPlayMenuData->QueueFull = 0;
                }
            }
    ...
    else if(evnet_type==APP_RECIVE_MSG_EVENT_WARING)
    {
        ...
        if (ret == RK_ERROR)
        {
            PLAYMENU_DEBUG ("-------lose MusicPlay_MsgBoxCallBack 0\n");
            rkos_queue_receive(gpstPlayMenuData->PlayMenuAskQueue, &PlayMenuAskQueue_tmp, 0);//MAX_DELAY
            PLAYMENU_DEBUG ("-------lose MusicPlay_MsgBoxCallBack 1\n");
            rkos_queue_send(gpstPlayMenuData->PlayMenuAskQueue, &PlayMenuAskQueue, 0);
        }
    }
    return RK_SUCCESS;
...
static COMMON FUN rk_err_t MusicPlay_SelectExePlayer(uint32 SourecePlayer)
{
    ...
    else if (gpstPlayMenuData->playerType == SOURCE_FROM_FILE_BROWSER)
    {
        if(MainTask_GetStatus(MAINTASK_APP_DLNA_PLAYER)
            || MainTask_GetStatus(MAINTASK_APP_BT_PLAYER)
            || MainTask_GetStatus(MAINTASK_APP_XXX_PLAYER))
        ...
        else if (MainTask_GetStatus (MAINTASK_APP_LOCAL_PLAYER))
        {
            ...
            if(gpstPlayMenuData->stAudioInfo.AudioSource == gpstPlayMenuData->playerType)
            {
                memcpy(Arg.filepath, gpstPlayMenuData->Arg.filepath, sizeof(Arg.filepath));
                //判断歌曲是否一样
                ...
                {
                    //Delete Local Player
                    MusicPlay_DeletePlayer(SOURCE_FROM_FILE_BROWSER);
                    return MusicPlay_StartPlayer(SOURCE_FROM_FILE_BROWSER);
                }
            }
        }
        else
        {
            ret = MusicPlay_StartPlayer(SOURCE_FROM_FILE_BROWSER);
            if(ret == RK_SUCCESS)
            {
                 MainTask_SetStatus(MAINTASK_APP_LOCAL_PLAYER, 1);
            }
            ...
            || (gpstPlayMenuData->playerType == SOURCE_FROM_XXX))
    {
        ...
        if(gpstPlayMenuData->playerType == SOURCE_FROM_DLNA)
        {
        ...
        else if(gpstPlayMenuData->playerType == SOURCE_FROM_XXX)
        {
        ...
        {
            ret = MusicPlay_StartPlayer(gpstPlayMenuData->playerType);
            if(ret == RK_SUCCESS)
            {
...
COMMON API int MusicPlayMenuTask_DeleteSelf(uint8 deleteplayer)
{
    ...
    if (deleteplayer == 1)
    {
        MusicPlay_DeletePlayer(gpstPlayMenuData->playerType);
        if((gpstPlayMenuData->playerType == SOURCE_FROM_FILE_BROWSER)
            || (gpstPlayMenuData->playerType == SOURCE_FROM_CUR_AUDIO))
        {
            MainTask_SetStatus(MAINTASK_APP_LOCAL_PLAYER, 0);
...
COMMON API void MusicPlayMenuTask_Enter(void)
{
...
#ifdef _USE_GUI_
    //Player Open
    MusicPlay_PlayerOpen(0); //若播放器打开失败，则不会再往下执行
    ...
    while (1)
    {
        if ((gSysConfig.PlayerType>SOURCE_FROM_NET)||(gSysConfig.PlayerType<SOURCE_FROM_FILE_BROWSER))
        {
            ...
            gpstPlayMenuData->playerType = gSysConfig.PlayerType;
            break;
        }
        rkos_sleep(1000);
    }
    #ifdef _BLUETOOTH_
    {
        ...
        #endif
        MusicPlay_StartPlayer(gpstPlayMenuData->playerType);
    }
    #endif
    ...
    #ifdef _WIFI_
    {
        #ifdef NOSCREEN_USE_LED
        {
            if (MainTask_GetStatus(MAINTASK_WIFICONFIG)!=1)
            {
                ...
                if (gpstPlayMenuData->playerType == SOURCE_FROM_XXX)
                {
                    MainTask_SetLED (MAINTASK_LED1,MAINTASK_LED_OFF);
                    MainTask_SetLED (MAINTASK_LED2,MAINTASK_LED_ON);
                    //test
        ...
        if (MainTask_GetStatus(MAINTASK_WIFICONFIG)==1)
        {
            gpstPlayMenuData->smartconfig = 1;
            rk_wifi_smartconfig();
            rk_printf("wifi(smart) config return-------------------------------\n");
            rkos_sleep(2000);
        }
        else
        {
            //while (wifi_join_flag() != WICED_TRUE)
            //{
                rkos_sleep(3000);
            //}
            if (gpstPlayMenuData->playerType != SOURCE_FROM_CUR_AUDIO)
    ...
    while(1)
    {
        ...
        switch (PlayMenuAskQueue.type)
        {
            case PLAYMENU_KEY_EVENT:
                ...
                gpstPlayMenuData->QueueFull = 0;
                #else
                printf ("-----No Screen Handle key\n");
                MusicPlay_NoScreenKeyHandle(PlayMenuAskQueue.cmd);
                gpstPlayMenuData->keycontrol = 0;
...
INIT API rk_err_t MusicPlayMenuTask_Init(void *pvParameters, void *arg)
{
    ...
    #ifdef _USE_GUI_
    hPlayMenuTaskData->playerType = stTaskPlayer->ucSelPlayType;
    #else
    hPlayMenuTaskData->playerType = gSysConfig.PlayerType;
    //rk_printf ("------------gSysConfig.PlayerType=0x%x\n",gSysConfig.PlayerType);
    #endif

    hPlayMenuTaskData->PlayMenuAskQueue = rkos_queue_create(3, sizeof(MUSIC_PLAY_MENU_ASK_QUEUE));
    hPlayMenuTaskData->PlayMenuRespQueue = rkos_queue_create(1, sizeof(MUSIC_PLAY_MENU_RESP_QUEUE));
    ...
    #ifndef _USE_GUI_
    {
        ...
        hPlayMenuTaskData->keycontrol = 0;
            ...
            MainTask_SetStatus(MAINTASK_APP_XXX_PLAYER_START, 0);
            MainTask_SetStatus(MAINTASK_APP_XXX_PLAYER, 0);
            hPlayMenuTaskData->AIRControl = 0;
    ...
    if ((hPlayMenuTaskData->playerType == SOURCE_FROM_CUR_AUDIO)
        ||(hPlayMenuTaskData->playerType == SOURCE_FROM_FILE_BROWSER))
    {
