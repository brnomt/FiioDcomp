...
typedef struct _FOLDER_INFO_STRUCT
{
    ...
    UINT16      CurItemInFatherCursor;                 /* in FatherCursor */
    UINT16      CurItemInFatherOffset;                 /* the serial number in father folder */
...
typedef  struct _BROWSERUI_TASK_DATA_BLOCK
{
    ...
    UINT16                      CurDirPath[259];                //direction path
    UINT16                      Pathlen;
    FOLDER_INFO_STRUCT          FolderInfo[MAX_DIR_DEPTH];      //
    BROWER_FILE_INFO            CurFileInfo;                    //file info
    //UINT16                      CurDisplayItem;
    UINT8                       *ExtName;
...
char  BrowserUIFileExtString[]   = "MP1MP2MP3WMAWAVAPELACAACM4AOGGMP43GPSBC";
...
uint8 BrowerUITask_GetFileType(UINT16 *path, UINT8 *pStr);
    ...
    if(StrLenW(gpstBrowserUITaskData->CurFileInfo.LongFileName) > 0)
    {
        ...
        *(gpstBrowserUITaskData->CurFileInfo.LongFileName + StrLenW(gpstBrowserUITaskData->CurFileInfo.LongFileName)) = 0x0000;
        ...
        BrowserUITask_GetFileOffset(gpstBrowserUITaskData->CurDirPath
                                    , gpstBrowserUITaskData->CurFileInfo.LongFileName
                                    , &(gpstBrowserUITaskData->FolderInfo[gpstBrowserUITaskData->CurDeep].CurItemInFatherOffset));

        if(gpstBrowserUITaskData->FolderInfo[gpstBrowserUITaskData->CurDeep].CurItemInFatherOffset >= gpstBrowserUITaskData->FolderInfo[gpstBrowserUITaskData->CurDeep].TotalItems)
        {
            gpstBrowserUITaskData->FolderInfo[gpstBrowserUITaskData->CurDeep].CurItemInFatherOffset = gpstBrowserUITaskData->FolderInfo[gpstBrowserUITaskData->CurDeep].TotalItems;
        }
       // printf("Offset:%d ",gpstBrowserUITaskData->FolderInfo[gpstBrowserUITaskData->CurDeep].CurItemInFatherOffset);
        if(gpstBrowserUITaskData->FolderInfo[gpstBrowserUITaskData->CurDeep].CurItemInFatherOffset > 7)
        {
            if(gpstBrowserUITaskData->FolderInfo[gpstBrowserUITaskData->CurDeep].CurItemInFatherOffset
                > (gpstBrowserUITaskData->FolderInfo[gpstBrowserUITaskData->CurDeep].TotalItems - 8))
            {
                gpstBrowserUITaskData->FolderInfo[gpstBrowserUITaskData->CurDeep].CurItemInFatherCursor =
                    (gpstBrowserUITaskData->FolderInfo[gpstBrowserUITaskData->CurDeep].CurItemInFatherOffset+ 8)
                    - gpstBrowserUITaskData->FolderInfo[gpstBrowserUITaskData->CurDeep].TotalItems;
                //printf(",>7,,Cursor = %d",gpstBrowserUITaskData->FolderInfo[gpstBrowserUITaskData->CurDeep].CurItemInFatherCursor);
            }
            else
            {
                gpstBrowserUITaskData->FolderInfo[gpstBrowserUITaskData->CurDeep].CurItemInFatherCursor = 0;
            }
        }
        else
        {
            gpstBrowserUITaskData->FolderInfo[gpstBrowserUITaskData->CurDeep].CurItemInFatherCursor =
                gpstBrowserUITaskData->FolderInfo[gpstBrowserUITaskData->CurDeep].CurItemInFatherOffset;
            //printf(",,,Cursor = %d",gpstBrowserUITaskData->FolderInfo[gpstBrowserUITaskData->CurDeep].CurItemInFatherOffset);
        }
    }
...
#ifdef __APP_BROWSER_BROWSERUITASK_C__
        RKTaskDelete(TASK_ID_BROWSERUI, 0, ASYNC_MODE);
        while(1)
        {
            rkos_sleep(2000);
            ...
            case BROWSER_UPDATA:
                {
                    if(gpstBrowserUITaskData->hSelect == NULL)
                    {
                        ...
                        //BrowserUITask_GetFilePathByCurNum(gpstBrowserUITaskData->FolderInfo[gpstBrowserUITaskData->CurDeep].CurItemInFatherOffset, CURDIR);
            ...
            case BROWSER_ENTER:
                if((browserUIAskQueue.offset + 1) <= gpstBrowserUITaskData->FolderInfo[gpstBrowserUITaskData->CurDeep].TotalDirs)
                {
                    if(gpstBrowserUITaskData->CurDeep < MAX_DIR_DEPTH)
                    {
                        ...
                        gpstBrowserUITaskData->FolderInfo[gpstBrowserUITaskData->CurDeep].CurItemInFatherCursor = browserUIAskQueue.cursor;
                        gpstBrowserUITaskData->FolderInfo[gpstBrowserUITaskData->CurDeep].CurItemInFatherOffset = browserUIAskQueue.offset;
    ...
    while(1)
    {
        ...
        if(StrCmpW((path + pathTempLen), (uint16 *)L"\\", 1) == RK_SUCCESS)
        {
            ...
            *(floderName + (pathLen - pathTempLen - 1)) = 0x0000;
            ...
            BrowserUITask_GetDirOffset(floderName, tempPath, &(gpstBrowserUITaskData->FolderInfo[deep].CurItemInFatherOffset));
            //printf("GetInfo[%d] Offset=%d \n",deep, gpstBrowserUITaskData->FolderInfo[deep].CurItemInFatherOffset);

            if(gpstBrowserUITaskData->FolderInfo[deep].CurItemInFatherOffset >= gpstBrowserUITaskData->FolderInfo[deep].TotalItems)
            {
                printf("GetInfo Offset:%d >= TotalItems:%d",gpstBrowserUITaskData->FolderInfo[deep].CurItemInFatherOffset
                    ,gpstBrowserUITaskData->FolderInfo[deep].TotalItems);
                goto err;
            }
            //printf("GetInfo Offset:%d ",gpstBrowserUITaskData->FolderInfo[deep].CurItemInFatherOffset);
            if(gpstBrowserUITaskData->FolderInfo[deep].CurItemInFatherOffset > 7)
            {
                if(gpstBrowserUITaskData->FolderInfo[deep].CurItemInFatherOffset
                    > (gpstBrowserUITaskData->FolderInfo[deep].TotalItems - 8))
                {
                    gpstBrowserUITaskData->FolderInfo[deep].CurItemInFatherCursor =
                        (gpstBrowserUITaskData->FolderInfo[deep].CurItemInFatherOffset+ 8)
                        - gpstBrowserUITaskData->FolderInfo[deep].TotalItems;
                    //printf("GetInfo ,,,Cursor = %d",gpstBrowserUITaskData->FolderInfo[deep].CurItemInFatherCursor);
                }
                else
                {
                    gpstBrowserUITaskData->FolderInfo[deep].CurItemInFatherCursor = 0;
                }
            }
            else
            {
                gpstBrowserUITaskData->FolderInfo[deep].CurItemInFatherCursor =
                    gpstBrowserUITaskData->FolderInfo[deep].CurItemInFatherOffset;
                //printf("GetInfo ,,,Cursor = %d",gpstBrowserUITaskData->FolderInfo[deep].CurItemInFatherOffset);
            }
        }
        if(pathTempLen < 3)
        {
        ...
        while(totalDirs--)
        {
            ...
            if(ret != RK_SUCCESS)
            {
                ...
                *offset = gpstBrowserUITaskData->FolderInfo[gpstBrowserUITaskData->CurDeep].TotalDirs - totalDirs - 1;
                gpstBrowserUITaskData->FolderInfo[gpstBrowserUITaskData->CurDeep].CurDirNum = *offset;
                return RK_ERROR;
            }
...
COMMON FUN rk_err_t BrowserUITask_ChangeDir(HDC hFather, UINT16 *pLongFileName, UINT16 *path, int inOutCur)
{
    ...
    if(inOutCur == NEXTDIR)
    {
        if(gpstBrowserUITaskData->CurDeep < MAX_DIR_DEPTH)
        {
            gpstBrowserUITaskData->CurDeep++;
            gpstBrowserUITaskData->FolderInfo[gpstBrowserUITaskData->CurDeep].CurItemInFatherCursor = 0;
            gpstBrowserUITaskData->FolderInfo[gpstBrowserUITaskData->CurDeep].CurItemInFatherOffset = 0;
            //rk_printf("++CurDeep =%d\n",gpstBrowserUITaskData->CurDeep);
        }
        else
        {
    ...
    if(gpstBrowserUITaskData->hDir[gpstBrowserUITaskData->CurDeep] == NULL)
    {
        ...
        if(gpstBrowserUITaskData->FolderInfo[gpstBrowserUITaskData->CurDeep].TotalDirs > 0)
        {
            ...
            {
                gpstBrowserUITaskData->FolderInfo[gpstBrowserUITaskData->CurDeep].CurDirNum = 0;
    ...
    if(gpstBrowserUITaskData->hDirForFlie[gpstBrowserUITaskData->CurDeep] == NULL)
    {
        ...
        if(gpstBrowserUITaskData->FolderInfo[gpstBrowserUITaskData->CurDeep].TotalFiles > 0)
        {
            ...
            {
                gpstBrowserUITaskData->FolderInfo[gpstBrowserUITaskData->CurDeep].CurFileNum = 0;
                //rk_printf("..FileDev_NextDir..\n");
            }
        }
    }
        ...
        while(totalFiles--)
        {
            ...
            if(ret != RK_SUCCESS)
            {
                ...
                *offset = gpstBrowserUITaskData->FolderInfo[gpstBrowserUITaskData->CurDeep].TotalItems - totalFiles - 1;
                ...
                gpstBrowserUITaskData->FolderInfo[gpstBrowserUITaskData->CurDeep].CurFileNum = curFileNum;
                return RK_ERROR;
            }
...
COMMON FUN rk_err_t BrowserUITask_StartPlayer(uint16 FileNum, int cmd)
{
    ...
    *(stTaskPlayer.filepath + gpstBrowserUITaskData->Pathlen + fileLen) = 0x0000;
    ...
    stTaskPlayer.ucSelPlayType = SOURCE_FROM_FILE_BROWSER;
    //rk_printf("\n.... music  FileNum =%d......\n",stTaskPlayer.FileNum);
...
*******************************************************************************/
_APP_BROWSER_BROWSERUITASK_COMMON_
COMMON FUN uint8 BrowerUITask_GetFileType(UINT16 *path, UINT8 *pStr)
{
    UINT8 Len;
    UINT8 Retval = 0xff;
    UINT8 i;
...
COMMON FUN rk_err_t BrowserUITask_GetFilePathByCurNum(uint32 curnum, int inOutCur)
{
    ...
    if(inOutCur == NEXTDIR)
    {
        ...
        bCurNum = gpstBrowserUITaskData->FolderInfo[gpstBrowserUITaskData->CurDeep].CurDirNum;
        if(curnum > bCurNum)
        {
        ...
        gpstBrowserUITaskData->FolderInfo[gpstBrowserUITaskData->CurDeep].CurDirNum = curnum;
...
*/
        ...
        bCurNum = gpstBrowserUITaskData->FolderInfo[gpstBrowserUITaskData->CurDeep].CurFileNum;
        if(curnum >= gpstBrowserUITaskData->FolderInfo[gpstBrowserUITaskData->CurDeep].TotalDirs)
        {
            ...
            gpstBrowserUITaskData->FolderInfo[gpstBrowserUITaskData->CurDeep].CurFileNum = fileNum;
        ...
        case SELECT_ENVEN_UPDATA:
        {
            ...
            //for(gpstBrowserUITaskData->CurDisplayItem; gpstBrowserUITaskData->CurDisplayItem < offset; gpstBrowserUITaskData->CurDisplayItem++)
            {
                        ...
                        if(offset > bCurNum)
                        {
                            for(i = 0; i < (offset - bCurNum); i++)
                            {
                                if (FileDev_NextDir(gpstBrowserUITaskData->hDir[gpstBrowserUITaskData->CurDeep], 0, NULL) != RK_SUCCESS)
                                {
                                    ...
                                    gpstBrowserUITaskData->FolderInfo[gpstBrowserUITaskData->CurDeep].CurDirNum = bCurNum + 1 + i;
                                    goto error;
                                }
                                else
                                {
                ...
                else
                {
                    ...
                    if(fileOffset > bCurNum)
                    {
                        for(i = 0; i < (fileOffset - bCurNum); i++)
                        {
                            if (FileDev_NextFile(gpstBrowserUITaskData->hDirForFlie[gpstBrowserUITaskData->CurDeep], 0, NULL, NULL) != RK_SUCCESS)
                            {
                                ...
                                gpstBrowserUITaskData->FolderInfo[gpstBrowserUITaskData->CurDeep].CurFileNum = bCurNum + 1 + i;
                                goto error;
                            }
                            else
                            {
                    ...
                    gpstBrowserUITaskData->FolderInfo[gpstBrowserUITaskData->CurDeep].CurFileNum = fileOffset;
                }
            }
...
INIT API rk_err_t BrowserUITask_Init(void *pvParameters, void *arg)
{
    ...
    RK_TASK_BROWSERUI_ARG * pArg = (RK_TASK_BROWSERUI_ARG *)arg;
    ...
    *(gpstBrowserUITaskData->CurDirPath + StrLenW(pArg->filepath)) = 0x0000;
    ...
    //printf("BrowserUITask_Init Success deep=%d len=%d Cursor=%d\n",gpstBrowserUITaskData->CurDeep, gpstBrowserUITaskData->Pathlen,gpstBrowserUITaskData->FolderInfo[gpstBrowserUITaskData->CurDeep].CurItemInFatherCursor);
    return RK_SUCCESS;
}
