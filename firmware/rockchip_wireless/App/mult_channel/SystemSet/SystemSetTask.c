...
COMMON API rk_err_t SystemSetTask_UpData(uint32 event_type, uint32 event, void * arg, uint32 mode)
{
    ...
    printf("updata SystemSetTask....... 0x%x\n",event);
...
COMMON API void SystemSetTask_Enter(void * arg)
{
    ...
    while (1)
    {
                                ...
                                if(gpstSystemSetData->hWait != NULL)
                                {
                                    ...
                                    /*
                                        if(RKTaskFind(TASK_ID_WIFI_APPLICATION, 0) != NULL )
                                        {
                                            ...
                                            MainTask_SetStatus(MAINTASK_WIFI_STATIC_ICON, 0);
                                            MainTask_SetIconStatus(MAINTASK_WIFI_ICON, 0);
                                            MainTask_SetTopIcon(MAINTASK_WIFIICON_DEL_CMD);
                                            MainTask_SetIconStatus(MAINTASK_WIFI_CONNECT_OK, 0);
                                            gSysConfig.SelPlayType = NULL;
                                            rk_printf("ESC Wifi_stop....\n");
                                            MainTask_SetIconStatus(MAINTASK_WIFI_OK, 0);
                                        }
                                    */
                                    }
                                    ...
                                    /*if(MainTask_GetStatus(MAINTASK_BT_OK) == 1)
                                    {
                                        ...
                                        MainTask_SetStatus(MAINTASK_BLUETOOTH_ICON, 0);
                                        MainTask_SetTopIcon(MAINTASK_BTICON_DEL_CMD);
        ...
        else
        {
            ...
            //printf("y = 0x%x",gpstSystemSetData->curCmd);

            if(MainTask_GetStatus(MAINTASK_WIFI_OK) == 1)
            {
                #ifdef _DRIVER_WIFI__
                if(wifi_connect_flag() == WICED_TRUE)
                {
                    if(MainTask_GetStatus(MAINTASK_WIFI_CONNECT_OK) == 0)
                    {
...
COMMON FUN rk_err_t System_SelectEventMove(int32 offset)
{
    ...
    if (gpstSystemSetData->blevel_set == 1)
    {
        ...
        if (hPwmDev != NULL)
        {
            PwmDev_SetRate(hPwmDev,BackLight_LevelValue[offset],10000);
            RKDev_Close(hPwmDev);
        }
        else
        {
...
COMMON FUN rk_err_t SystemTask_EvnetOper(uint32 menuTextID)
{
    ...
    switch (menuTextID)
    {
        ...
        case SID_WIFICONFIG_ON:
            ...
            #endif
            {
                ...
                if(RKTaskFind(TASK_ID_WIFI_APPLICATION, 0) == NULL )
                {
                    ...
                    MainTask_SetStatus(MAINTASK_WIFI_OK, 1);
                }
                ...
                if(gpstSystemSetData->curCmd != SID_WIFICONFIG_ON)
                {
                    ...
                    printf("create hWait = 0x%x",gpstSystemSetData->hWait);
        ...
        case SID_WIFICONFIG_OFF:
            ...
            if(RKTaskFind(TASK_ID_AIRPLAY, 0) != NULL )
            {
            ...
            if(RKTaskFind(TASK_ID_WIFI_APPLICATION, 0) != NULL )
            {
                ...
                gSysConfig.SelPlayType = NULL;
                rk_printf("$$$$Wifi_stop....\n");
                MainTask_SetStatus(MAINTASK_WIFI_OK, 0);
            }
        ...
        case SID_APCONFIG:
            ...
            if(MainTask_GetStatus(MAINTASK_WIFI_OK))
            {
                ...
                if(wifi_init_flag() == WICED_TRUE)
                {
                    ...
                    else
                    {
                        ...
                        else if(wifi_easy_setup_flag() == WICED_ERR)
                        {
                            if(gpstSystemSetData->hWait != NULL)
                            {
                                GuiTask_OperWidget(gpstSystemSetData->hWait, OPERATE_SET_DISPLAY, (void *)0, SYNC_MODE);
        ...
        case SID_SCREEN_OFF_30_MIN:
            ...
            gSysConfig.BTotalTime = BackLight_Time[gSysConfig.BLtime-1];
            /*System_GoToFatherList();*/
        ...
        case SID_LUM_5:
            ...
            if (hPwmDev != NULL)
            {
                ...
                PwmDev_SetRate(hPwmDev,BackLight_LevelValue[gSysConfig.BLevel-1],10000);
                RKDev_Close(hPwmDev);
                /*
...
COMMON FUN rk_err_t SystemTask_ChangeSelectList(uint32 SelectID, uint32 CursorID)
{
    ...
    if(SelectID != 0xFFFF)
    {
        ...
        if((SetMenuTxtInfo.ChildID == 0xFFFF) || (SelectID == SID_APCONFIG))
        {
