...
static void make_dhcpd_packet(struct dhcpd *packet, struct dhcpd *oldpacket, char type)
{
    ...
    char *dest = packet->chaddr;
    char *src = oldpacket->chaddr;
    memcpy(dest, src, 16);
...
static int request(struct dhcpd *packetinfo)
{
    ...
    if((lease = DHCPD_FindLeaseByChaddr(packetinfo->chaddr))!=NULL)
    {
        ...
        char *dest = payload_out.chaddr;
        char *src = packetinfo->chaddr;
        memcpy(dest, src, 16);
...
u8_t dhcp_process(void *packet_addr)
{
    char state;
...
void dhcpd_daemon( void )
{
    ...
    //leases=(struct dhcpOfferedAddr *)malloc(sizeof(struct dhcpOfferedAddr)*5);
