...
int easy_setup_start()
{
    ...
    result = &ws->result;
    JoinAp.ap_ssid_len = result->ap_ssid.len;
    rkos_memcpy(JoinAp.ap_ssid_value, result->ap_ssid.val, JoinAp.ap_ssid_len);
    JoinAp.security_key_length = result->security_key_length;
    rkos_memcpy(JoinAp.security_key, result->security_key, JoinAp.security_key_length);
    rkos_memcpy(JoinAp.ap_bssid, result->ap_bssid.octet, 6);
...
//   wifi_setup_security = ws->security;
    JoinAp.ap_security_type = ws->security;
