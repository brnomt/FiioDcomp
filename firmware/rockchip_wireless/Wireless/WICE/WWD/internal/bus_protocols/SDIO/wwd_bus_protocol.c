...
wwd_result_t wwd_bus_sdio_read_register_value    ( wwd_bus_function_t function, uint32_t address, uint8_t value_length, /*@out@*/ uint8_t* value );
...
//#ifndef WICED_DISABLE_MCU_POWERSAVE
    ...
    VERIFY_RESULT( wwd_bus_write_backplane_value( (uint32_t) SDIO_TO_SB_MAILBOX, (uint8_t) 1, (uint32_t)SMB_USE_OOB ) );
    ...
    printf(" sdio_download_firmware\n");

    VERIFY_RESULT( wwd_bus_sdio_download_firmware( ) );
    printf("sdio_download_firmware ok\n");
    ...
    WPRINT_WWD_INFO(("[WiFi] Chip Specific Init OK\n"));

    VERIFY_RESULT( wwd_ensure_wlan_bus_is_up() );
    WPRINT_WWD_INFO(("[WiFi] Wlan Bus Up\n"));
#endif
...
{
    return wwd_bus_write_backplane_value( (uint32_t) SDIO_INT_STATUS, (uint8_t) 4, (intstatus & I_HMB_SW_MASK /* Micky */));
}
...
uint32_t wwd_bus_packet_available_to_read(void)
{
    ...
    {
        WPRINT_WWD_ERROR(("[WiFi] Read INT status failed\n"));
    }

    /* Clear any interrupts */
    if ( wwd_bus_write_backplane_value( (uint32_t) SDIO_INT_STATUS, (uint8_t) 4, (int_status & I_HMB_SW_MASK /* Micky */) ) != WWD_SUCCESS )
    {
        WPRINT_WWD_ERROR(("[WiFi] Write INT status failed\n"));
    }
...
wwd_result_t wwd_bus_read_frame( /*@out@*/ wiced_buffer_t* buffer )
{
    ...
    *buffer = NULL;
    ...
    int size = (int) ( INITIAL_READ  + extra_space_required +  sizeof(wwd_buffer_header_t) );
    if (size > (unsigned short) WICED_LINK_MTU )
    {
        rk_printf("buffer = %x", *buffer);
        host_buffer_release(*buffer, WWD_NETWORK_RX);

        uint8_t *big_buf = NULL;

        big_buf = (uint8_t *)rkos_memory_malloc(size);
        if(big_buf == NULL)
        {
            rk_printf("big_buf malloc fail");
            return WWD_SDIO_RX_FAIL;
        }

        if ( extra_space_required > 0 )
        {
            result = wwd_bus_sdio_transfer(BUS_READ, WLAN_FUNCTION, 0, extra_space_required, big_buf, RESPONSE_NEEDED);
            if ( result != WWD_SUCCESS )
            {
                (void) wwd_bus_sdio_abort_read( WICED_FALSE ); /* ignore return - not much can be done if this fails */
            }
...
wwd_result_t wwd_bus_write_backplane_value( uint32_t address, uint8_t register_length, uint32_t value )
{
  ...
  // printf("re=%x", result);
   if(result != WWD_SUCCESS)
    ...
    {
        address |= SDIO_SB_ACCESS_2_4B_FLAG;
    }
...
static wwd_result_t wwd_bus_sdio_download_firmware( void )
{
    ...
    printf("fw and nvram down ok\n");


#ifdef _WIFI_5G_AP6255

    uint8 bus_buf[4] = {0x98, 0xF1, 0x3E, 0xB8};//{0x98, 0xF1, 0x3E, 0xB8};// 0x80, 0xF1, 0xA4, 0xBF, 0x80, 0xF1, 0xB0, 0xBF, 0x80, 0xF1, 0xBC, 0xBF, 0x80, 0xF1, 0xCB, 0xBF, 0x80, 0xF1, 0xDA, 0xBF, 0x80, 0xF1, 0xE9, 0xBF, 0x80, 0xF1, 0xF8, 0xBF};
    uint8 read_buf[4] = {0};
    uint32 address = 0x00;
    ...
    if ( WWD_SUCCESS != ( result = wwd_bus_transfer_bytes( BUS_WRITE, BACKPLANE_FUNCTION, address & BACKPLANE_ADDRESS_MASK, 4, (wwd_transfer_bytes_packet_t*)bus_buf ) ) )
    {
        return WWD_TIMEOUT;
    }

    result = wwd_bus_transfer_bytes( BUS_READ, BACKPLANE_FUNCTION, address & BACKPLANE_ADDRESS_MASK, 4, (wwd_transfer_bytes_packet_t*)read_buf );
    if(result == WWD_SUCCESS)
    {
        debug_hex(read_buf, 4, 16);
    }
    else
    {
        printf("fail\n");
...
wwd_result_t wwd_bus_poke_wlan( void )
{
    ...
    //return wwd_bus_write_backplane_value( SDIO_TO_SB_MAILBOX, (uint8_t) 4, (uint32_t)SMB_DEV_INT );
    //FW_SR
    return wwd_bus_write_backplane_value( SDIO_TO_SB_MAILBOX, (uint8_t) 4, (uint32_t)(SMB_DEV_INT | SMB_USE_OOB) );
}
...
static wwd_result_t wwd_bus_sdio_set_oob_interrupt( uint8_t gpio_pin_number )
{
    if ( gpio_pin_number == 1 )
    {
        ...
        VERIFY_RESULT( wwd_bus_write_register_value( BACKPLANE_FUNCTION, CHIPCOMMON_GPIO_CONTROL&0x07FFF, (uint8_t)4, (uint32_t)0x2 ) );
    }
